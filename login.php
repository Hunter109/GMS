...
if (isset($_POST['login'])) {
	...
	$memberID = validate_form($_POST['memberID']); // Sanitize adminID
	$password = md5(validate_form($_POST['password'])); // Sanitize and hash the password using md5

	$sql = "SELECT * FROM Members WHERE MemberID = '$memberID' AND LoginPassword = '$password' ";
	...
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()) {
			...
			header("Location: http://localhost/GMS/member/dashboard.php"); // Redirect after successful login
        ...
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md flex justify-between items-center mb-4">
            <span>
			We\'re sorry, but the member details you provided do not match our records. Please double-check the information you entered and try again. If you continue to face issues, please contact our support team for assistance.
...
<head>
	...
	<title>Member Login | Gymnasium Management System</title>
			...
			<div class="w-full max-w-xl ">
				<div class="md:bg-white md:shadow-md rounded-lg px-8 py-8">
					...
					<div class="text-center my-8">
						<h1 class="text-4xl font-bold text-black py-2">USER LOGIN</h1>
						<h2 class="text-2xl font-bold text-red-700 ">Welcome <span class="text-black">Back</span></h2>
					</div>
					<!-- USER FORM -->
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
						<div class="mb-4">
							<label for="username" class="block mb-2 text-sm  md:text-lg font-medium text-black">User
								ID</label>
							<input id="username" type="text" name="memberID" class="w-full px-3 py-2 placeholder-gray-500 border border-gray-300 rounded " placeholder="User ID">
						</div>
						<div class="mb-6">
						...
						<div class="flex items-center justify-between mb-6">
							<a href="./registration/memberforgot.php" class="text-black md:text-lg  hover:text-red-700">Forgot Password?</a>
					...
					<div class="text-center text-black md:text-lg">
						Not A Member? <a href="./trainer.php" class="text-red-700 font-bold hover:text-black">Trainer
							Login</a>
					</div>
					<div class="text-center text-black md:text-lg">
						New To GMS <a href="./registration/registration.php" class="text-red-700 font-bold hover:text-black">Register Here</a>
					</div>
				</div>
			</div>
		</div>

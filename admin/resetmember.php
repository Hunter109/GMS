...
$query = "SELECT DISTINCT MemberID ,Email FROM MemberPasswordReset";
...
<body class=" bg-gray-200">
                    ...
                    <table class="text-center text-sm font-light">
                        <thead class="border-b bg-neutral-900 font-medium text-white  ">
                            <tr>
                                ...
                                <th scope="col" class="px-[15px] py-5">Member ID</th>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td class='whitespace-nowrap px-[15px] py-5'>" . $row['MemberID'] . "</td>";
                                    echo "<td class='whitespace-nowrap px-[15px] py-5'>" . $row['Email'] . "</td>";

                                    echo "
                                <td class='py-2 px-4'>
                                    <a href='javascript:void(0);' onclick='showConfirmation(" . $row['MemberID'] . ")' class='text-white bg-red-500 hover:bg-red-700 py-3 px-12 rounded inline-flex items-center'>
    ...
    <div id="confirmationDiv" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.5); z-index: 999;">
        <div style="background: white; width: 400px; padding: 20px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; border-radius: 5px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.3);">
            <p style="font-size: 18px; margin-bottom: 20px;">Do you really want to reset this member password?</p>
            <button onclick="resetMember()" style="background-color: #e53e3e; color: white; padding: 10px 20px; border: none; border-radius: 5px; margin-right: 10px; cursor: pointer;">Yes</button>
    ...
    <script>
        var MemberReset = null;

        function showConfirmation(memberID) {
            MemberReset = memberID;
        ...
        function resetMember() {
            if (MemberReset !== null) {
                window.location.href = 'passwordresetmember.php?id=' + MemberReset;
            }
        }

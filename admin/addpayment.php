...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $amountPaid = validate_form($_POST['amount-to-pay']);
    $paymentDate = validate_form($_POST['paymentdate']);
    ...
    $checkUserQuery = "SELECT * FROM Payment WHERE MemberID = '$memberID'";

    $result = $conn->query($checkUserQuery);


    if ($result->num_rows > 0) {
        $addpayment = "UPDATE Payment SET AmountPaid = '$amountPaid', PaymentDate = '$paymentDate' WHERE MemberID = '$memberID'";
    } else {
        $addpayment = "INSERT INTO Payment (AmountPaid, PaymentDate, MemberID) VALUES ('$amountPaid', '$paymentDate', '$memberID')";
    }



    if ($conn->query($addpayment) === TRUE) {
        $alert = '
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-md flex justify-between items-center mb-4">
                <span>
                    Membership Payment Done successfully.
                </span>
        ...
        $alert = '
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md flex justify-between items-center mb-4">
                <span>
                    Error updating member details: ' . $conn->error . '
                </span>
...
<body>
    ...
    <div class="registration-container mt-24 ml-64">
        ...
        <div class="flex justify-center items-start p-10 md:h-screen">
            <div class="w-full max-w- md:bg-gray-200 md:p-8 rounded-md md:shadow-md">
                <div class="text-2xl font-semibold mb-4 ">Make Membership Payement</div>
                <div class="content">
                    <!-- UPDATE FORM -->
                    <form action="<?php echo $_SERVER['PHP_SELF'] . '?id=' . $memberID; ?>" method="POST">
                        <div class="user-details flex flex-wrap justify-between mb-4">
                            ...
                            <div class="input-box mb-3 w-full md:w-[49%]">
                                ...
                                <input type="date" name="dob" placeholder="Enter your date of birth" required readonly class="w-full px-3 py-2 placeholder-gray-500 border border-gray-300 rounded" value="<?php echo $row['Dob']; ?>" />
                            </div>
                            <div class="input-box mb-3 w-full md:w-[49%]">
                            ...
                            <div class="input-box mb-3 w-full md:w-[49%]">
                                ...
                                <input type="email" name="email" placeholder="Enter your email" required readonly class="w-full px-3 py-2 placeholder-gray-500 border border-gray-300 rounded" value="<?php echo $row['Email']; ?>" />
                            </div>

                            <div class="input-box mb-3 w-full md:w-[49%]">
                                <span class="text-sm md:text-lg font-medium">Membership Plan</span>
                                <select name="membership-plan" id="membership-plan" disabled class="w-full px-3 py-2.5 placeholder-gray-500 border border-gray-300 rounded">
                            ...
                            <div class="input-box mb-3 w-full md:w-[49%]">
                                <span class="text-sm md:text-lg font-medium">Amount to Pay</span>
                                <input type="text" id="amount-to-pay" name="amount-to-pay" class="w-full px-3 py-2.5 placeholder-gray-500 border border-gray-300 rounded" readonly>
                            </div>
                            <div class="input-box mb-3 w-full md:w-[49%]">
                                <span class="text-sm  md:text-lg font-medium">Date of Payement</span>
                                <input type="date" name="paymentdate" placeholder="Enter your date of birth" required class="w-full px-3 py-2 placeholder-gray-500 border border-gray-300 rounded" />
                            </div>
                        ...
                        <div class="gender-details flex items-center justify-start mb-4">
                            ...
                            <div class="category flex">
                                <label for="male" class="flex items-center cursor-pointer mr-4">
                                    <input type="radio" name="gender" value="Male" id="male" class="mr-1" disabled required <?php echo ($row['Sex'] === 'Male') ? 'checked' : ''; ?>>
                                    <span class="gender ">Male</span>
                                </label>
                                <label for="female" class="flex items-center cursor-pointer mr-4">
                                    <input type="radio" name="gender" value="Female" id="female" class="mr-1" disabled required <?php echo ($row['Sex'] === 'Female') ? 'checked' : ''; ?>>
                                    <span class="gender ">Female</span>
                                </label>
                            </div>
                        </div>
                        <div class="button">
                            <input type="submit" name="submit" value="Make Payment" class="h-10 w-full rounded text-black text-lg font-semibold bg-gray-300 hover:bg-black hover:text-white cursor-pointer" />
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        function changePageLocation() {
            window.location.href = './listpayment.php';
        }
    </script>
    ...
    <script>
        ...
        const selectedValue = "<?php echo $row['MembershipType']; ?>";
        ...
        if (selectedValue === silver) {
            amountToPayInput.value = '3000';
        } else if (selectedValue === golden) {
            amountToPayInput.value = '5000';
        } else if (selectedValue === platinum) {
            amountToPayInput.value = '10000';
        } else {

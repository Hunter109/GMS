...
$query = "SELECT MemberID, FullName, Sex, MobileNo, Email, Address, MembershipType, Dob, Age, StartDate, ExpiryDate FROM Members ORDER BY MemberID ASC ";
...
<body class=" bg-gray-200">
                    ...
                    <table class="text-center text-sm font-light">
                        <thead class="border-b bg-neutral-900 font-medium text-white  ">
                            <tr>
                                ...
                                <th scope="col" class="px-[13px] py-4">Start Date</th>
                                <th scope="col" class="px-[13px] py-4">Expiry Date</th>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td class='whitespace-nowrap px-[13px] py-4'>" . $row['StartDate'] . "</td>";
                                    echo "<td class='whitespace-nowrap px-[13px] py-4'>" . $row['ExpiryDate'] . "</td>";

                                    echo "
                                    <td class='py-2 px-4'>
                                    <a href='javascript:void(0);' onclick='showConfirmation(" . $row['MemberID'] . ")'
                                       class='text-white bg-red-500 hover:bg-red-700 py-2 px-4 rounded inline-flex items-center'>
                                        Pay <i class='fas fa-money-bill-wave ml-2'></i>
                                    </a>
                                </td>
                                ";
    ...
    <div id="confirmationDiv" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.5); z-index: 999;">
        <div style="background: white; width: 400px; padding: 20px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; border-radius: 5px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.3);">
            <p style="font-size: 18px; margin-bottom: 20px;">Do you wish to proceed with Membership Payment? </p>
            <button onclick="updateMember()" style="background-color: red; color: white; padding: 10px 50px; border: none; border-radius: 5px; margin-right: 10px; cursor: pointer;">Yes</button>
    ...
    <script>
        ...
        function updateMember() {
            if (memberupdate !== null) {
                window.location.href = 'addpayment.php?id=' + memberupdate;
            }
        }

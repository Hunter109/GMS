...
$query = "SELECT * FROM Payment ORDER BY PaymentID ASC ";
...
<body class=" bg-gray-200">
                    ...
                    <table class="text-center text-sm font-light">
                        <thead class="border-b bg-neutral-900 font-medium text-white  ">
                            <tr>
                                ...
                                <th scope="col" class="px-[13px] py-4">Payment ID</th>
                                <th scope="col" class="px-[13px]  py-4">Amount Paid</th>
                                <th scope="col" class="px-[13px] py-4">Payment Date</th>
                                <th scope="col" class="px-[13px] py-4"> MemberID</th>
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td class='whitespace-nowrap px-[13px] py-4'>" . $row['PaymentID'] . "</td>";
                                    echo "<td class='whitespace-nowrap px-[13px] py-4'>" . $row['AmountPaid'] . "</td>";
                                    echo "<td class='whitespace-nowrap px-[13px] py-4'>" . $row['PaymentDate'] . "</td>";
                                    echo "<td class='whitespace-nowrap px-[13px] py-4'>" . $row['MemberID'] . "</td>";

                                    echo "
                                <td class='py-2 px-4'>
                                <a href='javascript:void(0);' onclick='showConfirmation(" . $row['PaymentID'] . ")'
                                 class='text-white bg-green-500 hover:bg-green-700 py-2 px-4 rounded inline-flex items-center'>
                                    View Detail <i class='fas fa-eye  ml-2'></i>
                                </a>
                              </td>";
    ...
    <div id="confirmationDiv" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.5); z-index: 999;">
        <div style="background: white; width: 400px; padding: 20px; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; border-radius: 5px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.3);">
            <p style="font-size: 18px; margin-bottom: 20px;">Do you want to View Payment Detail of Membership"?</p>
    ...
    <script>
        ...
        function updateMember() {
            if (paydetail !== null) {
                window.location.href = 'viewpaiddetail.php?id=' + paydetail;
            }
        }

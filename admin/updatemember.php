...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows === 0 || $email === $row['Email']) {
        ...
        if ($password === $storedPassword) {
            ...
            $updateSql = "UPDATE Members SET FullName='$fullName', Dob='$dob', MobileNo='$phoneNumber', Address='$address',Age ='$age', StartDate ='$startDate',ExpiryDate = '$endDate', Email='$email', MembershipType='$membershipPlan', Sex='$gender' WHERE MemberID='$memberID'";
        } else {
            // Update the Member details in the database with new password 
            // Update the member details in the database
            $updateSql = "UPDATE Members SET FullName='$fullName', Dob='$dob', MobileNo='$phoneNumber', Address='$address',Age ='$age', StartDate ='$startDate',ExpiryDate = '$endDate', Email='$email', MembershipType='$membershipPlan', LoginPassword='$encpassword', Sex='$gender' WHERE MemberID='$memberID'";
        }
        ...
        if ($conn->query($updateSql) === TRUE) {
            $alert = '
                <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-md flex justify-between items-center mb-4">
                    <span>
                        Member Detail Updated successfully.
                    </span>
    ...
    <div class="registration-container mt-24 ml-64">
        ...
        <div class="flex justify-center items-start p-10 md:h-screen">
            <div class="w-full max-w- md:bg-gray-700 md:p-8 rounded-md md:shadow-md">
                <div class="text-2xl font-semibold mb-4 text-white">Update Member Details</div>
                <div class="content">
                    <!-- UPDATE FORM -->

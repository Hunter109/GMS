...
if (isset($_POST['submit'])) {
	...
	$checkUserQuery = "SELECT * FROM Trainers WHERE TrainerID = '$trainerid' AND Email = '$email'";
	...
	if ($result->num_rows > 0) {
		...
		try {
			...
			// Email content
			$mail->isHTML(true);
			$mail->Subject = 'New Trainer Password Reset Alert  Received (Forgot Password Trainer Page)';
			$mail->Body = "<h3> Trainer ID: $userid<br>Email :  $email</h3>";
			...
			$sql = "INSERT INTO TrainerPasswordReset (TrainerID, Email)
                    VALUES ('$trainerid', '$email')";
		...
		// Trainer details do not exist in the database, display an error message
		$alert = '
...
<head>
	...
	<title>Trainer Forgot | Gymnasium Management System</title>
			...
			<div class="w-full max-w-xl">
				<div class="md:bg-white md:shadow-md rounded-lg px-8 py-8">
					...
					<div class="text-center my-8">
						...
						<h2 class="text-2xl font-bold text-red-700">Recover Your Trainer Account</h2>
					</div>

					<!-- FORGOT PASSWORD FORM -->
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
						<div class="mb-4">
							<label for="username" class="block mb-2 text-sm md:text-lg font-medium text-black">Trainer ID</label>
					...
					<div class="text-center text-black md:text-lg">
						Remember your password? <a href="../trainer.php" class="text-red-700 font-bold hover:text-black">Sign In</a>
					</div>
					<div class="text-center text-black md:text-lg">

...
if (isset($_POST['submit'])) {
	...
	// Construct the SQL SELECT statement to check if the admin's details exist
	$checkUserQuery = "SELECT * FROM Admins WHERE AdminID = '$adminid' AND Email = '$email' AND MobileNo = '$phone' ";

	// Execute the SQL SELECT statement
	$result = $conn->query($checkUserQuery);

	if ($result->num_rows > 0) {
		// Admin's details exist in the database, proceed with password update

		// Construct the SQL Update statement
		$sql = "UPDATE Admins SET LoginPassword = '$encpassword' WHERE AdminID = '$adminid' AND Email = '$email' AND MobileNo = '$phone' ";
		...
		if ($conn->query($sql) === TRUE) {
			...
			$alert = '<div data-aos="zoom-in-up" class="forgot-password-alert py-1 m-4 rounded-lg bg-gray-300 h-auto">
                        <h2 class="text-center text-xl sm:text-2xl lg:text-4xl m-4 tracking-tight font-extrabold">Admin Password Reset<span class="text-red-700"> Confirmation</span></h2>
                        <div class="p-3 text-sm sm:text-base">
                            <p class="mb-4 text-black">Your admin password has been successfully reset!</p>
                            <p class="mb-4 font-medium">A new temporary password has been generated for your admin account.</p>
                            ...
                            <ol class="p-2 text-sm sm:text-base font-semibold">
                                <li class="ml-2 text-black">(1) Use the temporary password to log in to your admin account.</li>
                                <li class="ml-2 text-black">(2) Upon successful login, update your password to something secure.</li>
                            </ol>
                        </div>
                    </div>';
		} else {
		...
		// Admin details do not exist in the database, display an error message
		$alert = '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md flex justify-between items-center mb-4">
                    <span>We\'re sorry, but the Admin details you provided do not match our records. Please double-check the information you entered and try again.</span>
...
<head>
	...
	<title>Admin Forgot | Gymnasium Management System</title>
			...
			<div class="w-full max-w-xl">
				<div class="md:bg-white md:shadow-md rounded-lg px-8 py-8">
					...
					<div class="text-center my-8">
						...
						<h2 class="text-2xl font-bold text-red-700">Recover Your Admin Account</h2>
					</div>

					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
						<div class="user-details flex flex-wrap justify-between mb-4">
							<div class="input-box mb-3 w-full md:w-1/2">
								<span class="text-sm md:text-lg font-medium">Admin ID</span>
								<input type="text" name="username" placeholder="Enter your name" required class="w-full px-3 py-2 placeholder-gray-500 border border-gray-300 rounded" />
							</div>
							<div class="input-box mb-3 w-full md:w-1/2">
							...
							<div class="input-box mb-3 w-full md:w-1/2">
								<span class="text-sm md:text-lg font-medium">New Password</span>
								<input id="passwordInput" type="password" name="password" placeholder="Enter your New Password" required class="w-full px-3 py-2 placeholder-gray-500 border border-gray-300 rounded" />
								<div class="m-2">
						...
						<div class="button">
							<input type="submit" name="submit" value="Submit" class="h-10 w-full rounded text-black text-lg font-semibold bg-gray-200 hover:bg-black hover:text-white cursor-pointer">
						</div>

					</form>

					<div class="text-center text-black md:text-lg mt-2">
						Remember your password? <a href="../admin.php" class="text-red-700 font-bold hover:text-black">Sign In</a>
					</div>
